...
describe('Property Tests — Import/Export & Health Check (Tasks 8.4–8.8)', { timeout: 60_000 }, () => {
  beforeAll(async () => {
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (title, username, password, url) => {
            ...
            expect((entries[0].fields.get('Password') as kdbxweb.ProtectedValue).getText()).toBe(password);
  ...
  describe('Property 14: CSV import correctness', () => {
    ...
    const safeString = fc.string({ minLength: 1, maxLength: 30 })
      .map((s) => s.replace(/[,"\n\r]/g, '').trim())
      .filter((s) => s.length > 0);
        ...
        fc.property(
          ...
          (rows) => {
            ...
            const csvLines = rows.map(
              (r) => `${r.title},${r.username},${r.password},${r.url}`
            );
    ...
    it('rows with empty title or password are always skipped', () => {
      const safeStr = fc.string({ minLength: 1, maxLength: 20 })
        .map((s) => s.replace(/[,"\n\r]/g, '').trim())
        .filter((s) => s.length > 0);
