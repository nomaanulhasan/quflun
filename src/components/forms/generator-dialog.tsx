...
export function GeneratorDialog({ onInsert }: GeneratorDialogProps) {
  ...
  const [config, setConfig] = useState<PasswordGeneratorConfig>(passwordGenerator.getDefaultConfig());
      ...
      <DialogContent className="max-w-sm p-6">
        ...
        <div className="mt-4 space-y-4">
          ...
          <div className="rounded-md border border-border bg-muted/50 p-3">
            <code className="break-all text-sm font-mono">{generated || '...'}</code>
          </div>
          ...
          <div className="flex gap-2">
            ...
            <Button type="button" size="sm" onClick={handleInsert} disabled={!generated} className="ml-auto">
              Use Password
            </Button>
          </div>
        </div>
